...
ServerOSC::ServerOSC(const quint16 &portNb) : Port(portNb){
...
/*
            ...
            if(launchAnalyze) {
                for(int k=0; k<listMovements->size();k++) { // chaque Mvt
                    for (int j=0;j<listMovements->at(k)->getListJointsMvt()->size();j++) { // chaque JointMvt
                        if (listMovements->at(k)->getListJointsMvt()->at(j)->getJointRef()->getNom() == "lefthand")
                            qDebug() << "Before : Last buffer JointRef lefthand dX : " << listMovements->at(k)->getListJointsMvt()->at(j)->getJointRef()->getBufferPositions()->last()->getDx() << endl;
                    }
                }
